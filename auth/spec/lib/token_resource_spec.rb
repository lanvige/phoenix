...
describe phoenix::TokenResource do
  let(:order) { phoenix::Order.new }
  let(:permission) { mock_model(phoenix::TokenizedPermission) }

  it 'should add has_one :tokenized_permission relationship' do
    assert phoenix::Order.reflect_on_all_associations(:has_one).map(&:name).include?(:tokenized_permission)
  end
